...
if args.model == "RNN":
    ...
    model.load_state_dict(torch.load("models/RNN/best_params.pt", map_location='cpu'))
elif args.model == "GRU":
    model = GRU(args.emb_size, args.hidden_size, 0, args.batch_size, vocab_size, args.num_layers, 1)
    model.load_state_dict(torch.load("models/GRU/best_params.pt", map_location='cpu'))
else:
...
# random seed
seed = torch.randint(0, model.vocab_size, (args.batch_size,))
...
for sample in samples:
    print(" ".join([id_2_word[idx] for idx in sample.numpy()]))
